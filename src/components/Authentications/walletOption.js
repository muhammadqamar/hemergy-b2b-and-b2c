  ...
  return (
    <div className="registration-box">
      <div className="flex-box d-column gap-x-sm">
        {connector?.name ? (
          <h3 className="p-xl center-text">Connected to {connector.name}</h3>
        ) : (
          <h3 className="p-xl center-text">Connect Your Wallet</h3>
        )}
      </div>
          ...
          onClick={async () => {
            if (isConnected) {
              ...
              if (userDetail?.walletAddresses) {
                resulter = [
                  ...userDetail?.walletAddresses,
                  { provider: connector?.name, value: address },
                ];
              } else {
      ...
      <p className="font-medium text-center p-sm text-textcolor">
        Skip for now
      </p>
    </div>
  );
